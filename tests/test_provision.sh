...
setup() {
  mkstub uapi 'echo "uapi $*" >> "$UAPI_LOG"
    case "$*" in
      *get_restrictions*)     echo "{\"result\":{\"data\":{\"prefix\":\"compte_\",\"max_username_length\":16},\"status\":1}}";;
...
test_cree_base_utilisateur_et_env_de_production() {
  setup; APP_URL=https://api.ex.com DB_NAME=monapp run bash "$SCRIPT"
  assert_eq "$RC" 0
  assert_eq "$(env_val APP_ENV)" production; assert_eq "$(env_val APP_DEBUG)" false; assert_eq "$(env_val APP_URL)" https://api.ex.com
  assert_eq "$(env_val APP_NAME)" MonApp                       # valeurs de l'exemple conservées
  ...
  assert_eq "$(env_val DB_DATABASE)" compte_monapp; assert_eq "$(env_val DB_USERNAME)" compte_monapp
  pw="$(env_val DB_PASSWORD)"; [ "${#pw}" -eq 24 ] || fail "mot de passe de 24 caractères attendu, obtenu ${#pw}"
  assert_file_contains "$UAPI_LOG" "create_database name=compte_monapp"
  assert_file_contains "$UAPI_LOG" "set_privileges_on_database user=compte_monapp database=compte_monapp"
}
test_le_mot_de_passe_n_est_jamais_affiche_et_l_env_est_en_600() {
  setup; DB_NAME=monapp run bash "$SCRIPT"
...
test_ne_touche_a_rien_si_env_existe() {
  ...
  DB_NAME=monapp run bash "$SCRIPT"
...
test_prefixe_cpanel_non_double_et_utilisateur_tronque() {
  setup; DB_NAME=compte_application_tres_longue run bash "$SCRIPT"
  assert_eq "$(env_val DB_DATABASE)" compte_application_tres_longue
  assert_eq "$(env_val DB_USERNAME)" compte_applicati           # 16 caractères max
}
test_base_existante_reutilisee_avec_avertissement() {
  setup; DB_EXISTS=1 DB_NAME=monapp run bash "$SCRIPT"
  assert_eq "$RC" 0; assert_contains "$OUT" "existe déjà : réutilisée"; assert_eq "$(env_val DB_DATABASE)" compte_monapp
}
test_utilisateur_existant_ne_fabrique_pas_de_faux_mot_de_passe() {
  setup; USER_EXISTS=1 DB_NAME=monapp run bash "$SCRIPT"
...
test_refus_de_creation_echoue_sans_ecrire_d_env() {
  setup; DB_DENY=1 DB_NAME=monapp run bash "$SCRIPT"
...
test_sans_uapi_cree_quand_meme_l_env_avec_avertissement() {
  setup; rm "$T/bin/uapi"; DB_NAME=monapp run bash "$SCRIPT"
